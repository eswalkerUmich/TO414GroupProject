...
linear_airbnb <- lm(log_price ~., data = airbnb_train)
summary(linear_airbnb)

linear_pred <- predict(linear_airbnb, newdata = airbnb_test)

# Result
linear_result <- postResample(linear_predict, airbnb_test$log_price)
linear_result
```
...
svm_result <- postResample(airbnb_svm_pred, airbnb_test$log_price)
svm_result

# Improving Model
airbnb_svm_improved <- ksvm(log_price ~., data = airbnb_train,
                          kernel = "laplacedot")

airbnb_svm_pred_improved <- predict(airbnb_svm_improved, airbnb_test)

svm_result_improved <- postResample(airbnb_svm_pred_improved, airbnb_test$log_price)
svm_result_improved

# Kernel = polydot: Rsquared = 0.7905900
# Kernel = laplacedot: Rsquared = 0.7988569
# Kernel = besseldot: Rsquared = 0.7687704
```
...
```{r}
combined_pred <- data.frame(linear_pred, knn_pred, dt_pred, ann_pred, airbnb_svm_pred_improved, airbnb_test$log_price)
